...
G와 J는 서로 다른 언어 저장소에서 독립 진행했고 R은 두 결과를 검토했다. Go는 추가 읽기 전용 검토자가 수정 후 회귀 테스트까지 실행했다. 세부 내용은 [Go 문서](https://github.com/hwain-ai/SENTINEL_GO/blob/main/docs/go-mutesting-adapter.md), [Java 실행 문서](https://github.com/hwain-ai/SENTINEL_JAVA/blob/main/docs/pit-execution-probe.md), [Java 보고서 계약](https://github.com/hwain-ai/SENTINEL_JAVA/blob/main/docs/pit-report-adapter.md)에 둔다.
